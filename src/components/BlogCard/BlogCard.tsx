...
const BlogCard:React.FC<BlogCardProps> = ({id, title, image, category}) => {
    const navigate = useNavigate()
    return (
        <div 
            className="hover:scale-[1.05] hover:cursor-pointer hover:shadow-[0_35px_35px_rgba(0,0,0,0.25)] hover:bg-transparent hover:rounded-[8px]"
            onClick={() => navigate(`/blog/${title.toLowerCase().replace(/\s+/g, '-')}`, {state: { blogId: id }})}
        >
